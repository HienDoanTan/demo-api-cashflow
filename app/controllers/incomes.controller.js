            ...
            include: [
                {
                    ...
                    on: {
                        attachedString: Sequelize.where(Sequelize.col('Incomes.AttachedString'), '=', Sequelize.col('attached_group.AttachedString')),
                    },
...
exports.insert = (req, res) => {
    User.findOne({
        ...
        const jane = await Incomes.bulkCreate(
            xObject
        );
...
exports.delete = (req, res) => {
    User.findOne({
        ...
        Incomes.update({UpdateDate: moment(moment.now()).add(7, 'hours').format(), UpdateBy: user.id, Status: false}, {
            where: {
                Id: req.body
            }
        }).then(() => {
